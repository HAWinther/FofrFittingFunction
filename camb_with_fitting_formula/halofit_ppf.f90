...
101         continue
    ...
    use FittingFormulaLinear, only: pofk_enhancement_linear
    type(MatterPowerData) :: CAMB_Pk
    integer :: itf, i
    real(dl) :: zz, kk, ratio, ratio_lin
    
    if(CP%fofr0>0.0d0) then
      do itf = 1, CAMB_Pk%num_z
        zz = CAMB_Pk%Redshifts(itf)
        do i=1, CAMB_Pk%num_k
          kk = exp(CAMB_Pk%log_kh(i))
          ratio_lin = pofk_enhancement_linear(zz,CP%fofr0,kk)
    
          ! Transform P(k) from linear f(R) to linear LCDM
          CAMB_Pk%matpower(i,itf) = CAMB_Pk%matpower(i,itf) - log(ratio_lin)
    ...
    if(CP%fofr0>0.0d0) then
      do itf = 1, CAMB_Pk%num_z
        ...
        do i=1, CAMB_Pk%num_k
          ...
          ! Factor to transform from linear f(R) to non-linear f(R)
          CAMB_Pk%nonlin_ratio(i,itf) = CAMB_Pk%nonlin_ratio(i,itf) * dsqrt(ratio / ratio_lin)
        
          
          ! Transform P(k) from linear LCDM to linear f(R)
          CAMB_Pk%matpower(i,itf) = CAMB_Pk%matpower(i,itf) + log(ratio_lin)
        end do
      end do
    endif
